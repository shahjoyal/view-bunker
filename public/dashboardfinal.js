// dashboard.js (sidebar + overview + single-bunker view)
// Uses: GET /api/blend/latest and GET /api/coal
...
function renderBunkerIntoSVG(svg, bunkerData, coalDB, bunkerIndex = 0, strokeOpenTop = true, strokeWidth = 1.3){
  ...
  const clipId = `clip-bunker-${bunkerIndex}-single`;
...
function renderOverview(blend, coalDB){
  ...
  // ensure overview & top overlay visible
  document.getElementById('overviewView').style.display = '';
  document.getElementById('singleView').style.display = 'none';
  document.getElementById('topOverlay').style.display = '';
}
...
function renderSingle(bunkerIndex, blend, coalDB){
  ...
  // reposition top overlay: center arrow to point to single bunker (approx center)
  // keep top line visible but arrows we can hide except one centered arrow
  const topOverlay = document.getElementById('topOverlay');
  topOverlay.style.display = ''; // keep line if you want; arrow will be centered
  // hide all arrows then show one centered near middle
  const arrows = topOverlay.querySelectorAll('.arrow');
  arrows.forEach(a => a.style.display = 'none');
  // place one arrow roughly centered (50% across full width)
...
async function refreshAndRender(activeMode, activeIndex){
  ...
  if(activeMode === 'overview'){
    // ensure all arrows visible (overview)
    const topOverlay = document.getElementById('topOverlay');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.sidebar .item').forEach(it => {
    it.addEventListener('click', async (e) => {
      ...
      // render according to selected tab
